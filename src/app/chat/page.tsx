...
export default async function ChatPage(props: { searchParams: Promise<SP> }) {
  ...
  let userData;
  try {
    const meRes = await apiServerFetch<{ success: boolean; data: any }>("/me");
  ...
  try {
    const [convsRes, messagesRes] = await Promise.allSettled([
      apiServerFetch<{ success: boolean; data: any[] }>("/chat/conversations"),
      activeId
        ? apiServerFetch<{ success: boolean; data: any[] }>(
    ...
    const validActiveId = activeConv ? activeId : conversations[0]?.id || null;
